...
#   endif // HARDWARE_LDPC
  ...
  _invMatrix = new int[bitPlaneLength*bitPlaneLength];

  for (int i = 0; i < bitPlaneLength*bitPlaneLength; i++)
...
//decode() finds the minimum rate for which the decoded bitstream matches
//the transmitted portion of the accumulated syndrome.
//The number of residual bit errors is also calculated.
void LdpcaDec::decode(double        *LLR_intrinsic,
                      double        *accumulatedSyndrome,
                      double        *source,
                      double        *decoded,
                      double        *rate,
                      double        *numErrors,
                      ...
                      int            numcode)
{
  ...
  for (code = numcode-2; code < numCodes; code++) {
    ...
    for (k = 1; k < m; k++) {
      ...
      syndrome[k] = (double)(((int)(accumulatedSyndrome[currIndex] +
                                    accumulatedSyndrome[prevIndex])) % 2);
    }
...
# if INVERSE_MATRIX
    if (rate[0] == 1) {
      ...
      for (int j = n-1; j > 0; j--) {
        ...
        if (accumulatedSyndrome[j] != 0)
          accumulatedSyndrome[j] = 1;
      }
...
int LdpcaDec::beliefPropagation(int    *ir,
                                int    *jc,
                                int     m,
                                int     n,
                                int     nzmax,
                                double *LLR_intrinsic,
                                double *syndrome,
                                double *decoded)
{
  int iteration;
  ...
  double *LLR_extrinsic;
  double *check_LLR;
  double *check_LLR_mag;
  double *rowTotal;
  double *LLR_overall;
