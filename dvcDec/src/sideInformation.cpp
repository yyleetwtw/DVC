...
*/
void SideInformation::createSideInfo(imgpel* imgPrevKey, imgpel* imgNextKey,
                                     imgpel* imgCurrFrame, int wzCurrNum, int CSS_mode,
                                     FILE* fBicupPtr, FILE* fSiPtr)
{
  int width  = _codec->getFrameWidth();
  int height = _codec->getFrameHeight();
...
{
    createSideInfo_MR_tmp(imgCurrFrame, wzCurrNum, fBicupPtr, fSiPtr);
}
}

void SideInformation::createSideInfo_MR_tmp(imgpel* imgSI, int currWzNum, FILE* fBicupPtr,
                                            FILE* fSiPtr){
    ...
    imgBicup = new imgpel[width*height];

    fseek(fBicupPtr, currWzNum*width*height, SEEK_SET);
    fread(imgBicup, width*height, 1, fBicupPtr);
    fseek(fSiPtr, currWzNum*width*height, SEEK_SET);
    fread(imgSI, width*height, 1, fSiPtr);

    //_model->correlationNoiseModeling_MR_tmp(currWzNum);
    _model->correlationNoiseModeling(imgSI, imgBicup);
