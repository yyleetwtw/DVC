...
/* iBand the SI consideration, might be DCTSI or QDCTSI */
  int iSign;
  int c;
  int lower;
  int upper;
...
/*
  DC term : mode 2 with si=DCTSI
  AC term : msb : mode 1 with si=QDCTSI
            non-msb : mode 2 with si=DCTSI
*/
  double p0,p1;
  double p=0.0;
  double tp=0.0;
...
/*
    ifstream offlineAlpha("./alpha_dis.txt"); //ddd
    double tmp;
    for(int i=0;i<(wzCurrNum-1)/2;i++)
        for(int j=0;j<352*288;j++)
            offlineAlpha>>tmp;
    for(int j=0;j<iHeight;j++)
        for(int i=0;i<iWidth;i++)
        {
            iIndex=i+j*iWidth;
            offlineAlpha >> dAlpha[iIndex];
        } //ddd
*/
/*
  for(int j=0;j<4;j++)for(int i=0;i<4;i++){ //ddd
    cout << dAlpha[j*iWidth+i] ; if(i!=3) cout<<','; else cout <<endl; //ddd
  }
  for(int j=0;j<4;j++)for(int i=0;i<4;i++){ //ddd
    cout << _codec->getQuantStep(i,j) ; if(i!=3) cout<<','; else cout <<endl; //ddd
  }
*/
  delete [] residue;
  delete [] residue_b;
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
void CorrModel::correlationNoiseModeling_MR_tmp(int currWzNum){

    ifstream offlineAlpha("./alpha.txt");
    double tmp;
    double alphaOff[16];
    double* dAlpha = _codec->getAlpha();
    int iWidth,iHeight,iIndex;
    iWidth = _codec->getFrameWidth();
    iHeight= _codec->getFrameHeight();

    for(int i=0;i<currWzNum;i++)
        for(int j=0;j<352*288;j++)
            offlineAlpha>>tmp;
