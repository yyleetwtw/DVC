...
const float Transform::CosTable[16] = {
1.0,
0.923879532511,
0.707106781187,
0.382683432365,
...
// -----------------------------------------------------------------------------
// PART 1 - Transform class core functions
// -----------------------------------------------------------------------------

// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
Transform::Transform(Codec *codec)
{
  _codec = codec;

  _blockSize = Codec::BlockSize;

  _srcBuffer = new int*[_blockSize];
  _dstBuffer = new int*[_blockSize];

  for (int i = 0; i < _blockSize; i++) {
    _srcBuffer[i] = new int[_blockSize];
    _dstBuffer[i] = new int[_blockSize];
  }
}

// -----------------------------------------------------------------------------
// PART 2 - DCT transform functions
// -----------------------------------------------------------------------------
...
# if INTEGER_DCT
      for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++)
          _srcBuffer[j][i] = (int)src[(x+i)+(y+j)*width];

      forward4x4(_srcBuffer, _dstBuffer, 0, 0);

      for (int j = 0; j < 4; j++)
        for (int i = 0; i < 4; i++) {
          int sign = (_dstBuffer[j][i] >= 0) ? 1 : -1;
          int idx  = (x+i) + (y+j)*width;

          // First scale up the intermediate result
          dst[idx] = abs(_dstBuffer[j][i]) * DctScaler[j][i];
          // Scale down with round up
          dst[idx] = (dst[idx] + (0x1<<(DctShift[j][i]-1))) >> DctShift[j][i];
          dst[idx] = dst[idx] * sign;
        }
# else // if !INTEGER_DCT
      dct4x4(src, dst, x, y);
# endif // INTEGER_DCT
...
void Transform::dct4x4(T *src, U *dst, int x, int y)
{
  ...
  for (int v = y; v < y+4; v++) {
    ...
    for (int i = x; i < x+4; i++) {
      ...
      for (int j = y; j < y+4 ;j++)
        //t += c*(float)cosf((2*(j-y)+1)*(v-y)*PI/8.0)*(float)src[i+j*width];
        t += c*CosTable[(2*(j-y)+1)*(v-y)%16]*(float)src[i+j*width];
  ...
  for (int u = x; u < x+4; u++) {
    ...
    for (int v = y; v < y+4; v++) {
      ...
      for (int i = x; i < x+4; i++)
        ...
        t += c*CosTable[(2*(i-x)+1)*(u-x)%16]*buffer[i+v*width];
...
// -----------------------------------------------------------------------------
// PART 3 - Inverse DCT transform functions
// -----------------------------------------------------------------------------
  ...
  for (int v = y; v < y+4; v++) {
    for (int i = x; i < x+4; i++) {
      ...
      for (int j = y; j < y+4; j++) {
        ...
        t += c*CosTable[(2*(v-y)+1)*(j-y)%16]*(float)src[i+j*width];
      }
  ...
  for (int u = x; u < x+4; u++) {
    for (int v = y; v < y+4; v++) {
      ...
      for (int i = x; i < x+4; i++) {
        ...
        t += c*CosTable[(2*(u-x)+1)*(i-x)%16]*buffer[i+v*width];
      }
...
void Transform::quantization(int *src, int *dst)
{
  ...
  for (int y = 0; y < height; y += _blockSize)
    for (int x = 0; x < width; x += _blockSize)
      quan4x4(src, dst, x, y);
}
...
# endif // RESIDUAL_CODING
      ...
      {
        if (src[index]/_codec->getQuantStep(i, j)>=0)
          dst[index] = (src[index])/_codec->getQuantStep(i, j);
        else //Quantized result < 0 case
        {
          dst[index] = abs(src[index])/_codec->getQuantStep(i, j);
          dst[index] |= (0x1 << (_codec->getQuantMatrix(qp, i, j)-1));
        }
...
// -----------------------------------------------------------------------------
// PART 5 - Inverse quantization functions
// -----------------------------------------------------------------------------
